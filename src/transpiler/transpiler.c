...
typedef struct {
    ...
    lang_ops_t       ops;
    hashmap_t*       var_types;    // name → "int"/"double"/"string"
    hashmap_t*       declared_vars;// C/C++ only: vars already emitted inline
    string_t*        out;
    int              indent;
    int              tmp_count;
} transpiler_t;
...
static void hmap_set_cstr(hashmap_t* map, const char* key, const char* val) {
...
static const char* hmap_get_cstr(hashmap_t* map, const char* key) {
    string_t* k = string_create_from(key);
...
static bool hmap_has_cstr(hashmap_t* map, const char* key) {
    string_t* k = string_create_from(key);
...
static void collect_vars(transpiler_t* ctx, TSNode node) {
...
static void collect_swap_temps(transpiler_t* ctx, TSNode node) {
    if (ts_node_is_null(node)) return;
    const char* type = ts_node_type(node);
...
static var_type_t infer_expr_type(transpiler_t* ctx, TSNode node) {
